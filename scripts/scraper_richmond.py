...
"""
Richmond Times-Dispatch Column.us Foreclosure Scraper
------------------------------------------------------
Pulls trustee sale notices from richmond.column.us.

Covers: Richmond City, Chesterfield, Henrico, Hanover

Separate pipeline from Fredericksburg/Culpeper — do NOT merge.
Writes to: data/foreclosures_richmond.json

Run:   python3 scripts/scraper_richmond.py
Then:  python3 scripts/sheets_sync.py --file data/foreclosures_richmond.json
Or:    bash scripts/update_richmond.sh
"""
...
# ── Config ──────────────────────────────────────────────────────────────────
COLUMN_US_URL = "https://richmond.column.us/search?noticeType=Foreclosure+Sale"
PAPER_HEADER  = "RICHMOND TIMES DISPATCH"
SOURCE_TAG    = "column_us_richmond"

SCRIPT_DIR    = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT  = os.path.join(SCRIPT_DIR, "..")
DATA_FILE     = os.path.join(PROJECT_ROOT, "data", "foreclosures_richmond.json")
...
def scrape() -> list:
    """Scrape richmond.column.us for Foreclosure Sale notices."""
    listings = []
    ...
    try:
        with sync_playwright() as pw:
            ...
            log.info(f"  Richmond: loading {COLUMN_US_URL}")
            ...
            try:
                ...
                log.warning(
                    f"  Richmond: page never showed '{PAPER_HEADER}' after 20s "
            ...
            while True:
                try:
                    ...
                    if clicked:
                        load_more_clicks += 1
                        log.info(f"  Richmond: clicked 'Load more' ({load_more_clicks}x)")
                        page.wait_for_timeout(2500)
                    else:
                        log.info(f"  Richmond: 'Load more' exhausted after {load_more_clicks} click(s)")
                        break
                except Exception as ex:
                    log.debug(f"  Richmond: 'Load more' loop ended: {ex}")
                    break
            ...
            try:
                ...
                log.debug(f"  Richmond: could not extract notice URLs: {e}")
                notice_urls = []

            log.info(f"  Richmond: {len(notice_urls)} individual notice URL(s) found")
            ...
            log.info(f"  Richmond: {total_blocks} total listings found")
        ...
        log.error(f"  Richmond error: {e}", exc_info=True)

    log.info(f"  Richmond: found {len(listings)} listings total")
    return listings
...
def run():
    log.info("Starting Richmond Times-Dispatch (richmond.column.us) scraper…")
    listings = scrape()
    save(listings)
    log.info("Done.")
